...
exports.getApprovedFarmerListings = async(req,res)=>{try{const fpoId=await getFpoIdForUser(req.user._id||req.user.id);if(!fpoId)return res.json([]);const rows=await Produce.find({requestedFpo:fpoId,fpoApprovalStatus:{$in:['Pending','Approved']}}).populate('farmerId','name email location');res.json(rows);}catch(e){res.status(500).json({message:e.message});}};
exports.approveFarmerListing = async(req,res)=>{try{const fpoId=await getFpoIdForUser(req.user._id||req.user.id);const p=await Produce.findOne({_id:req.params.produceId,requestedFpo:fpoId});if(!p)return res.status(404).json({message:'Farmer listing not found or not requested from this FPO.'});const status=req.body.status||'Approved';if(!['Pending','Approved','Rejected'].includes(status))return res.status(400).json({message:'Invalid approval status.'});p.fpoApprovalStatus=status;p.approvedByFpo=fpoId;p.fpoApprovalReason=req.body.reason||'';p.fpoApprovedAt=status==='Approved'?new Date():undefined;await p.save();res.json(p);}catch(e){res.status(400).json({message:e.message});}};
exports.requestFarmerListingApproval=async(req,res)=>{try{const p=await Produce.findOne({_id:req.params.produceId,farmerId:req.user._id||req.user.id});if(!p)return res.status(404).json({message:'Your produce listing was not found.'});if(!req.body.fpoId)return res.status(400).json({message:'fpoId is required.'});p.requestedFpo=req.body.fpoId;p.fpoApprovalStatus='Pending';await p.save();res.json({message:'FPO approval requested.',produce:p});}catch(e){res.status(400).json({message:e.message});}};
