...
router.post('/', protect, authorizeRoles('fpo_admin', 'fpo_staff'), uploadLimiter, upload.array('images', 5), c.createListing);
router.patch('/:id/status', protect, authorizeRoles('fpo_admin', 'fpo_staff'), requireVerifiedKycToPublish, c.setListingStatus);
router.put('/:id', protect, authorizeRoles('fpo_admin', 'fpo_staff'), uploadLimiter, upload.array('images', 5), c.updateListing);
