...
if (allowedOrigins.length) {
  ...
  // Fail closed: refuse to start in production rather than silently opening
  // the API to every origin. A missing env var should break loudly here,
  // not become an open-CORS API with only a log line to notice it by.
  fatal('CORS_ORIGIN is not set. Refusing to start in production with CORS open to every origin.');
} else {
  console.warn('[security] CORS_ORIGIN is not set: any website can call this API (dev only).');
  app.use(cors());
}
